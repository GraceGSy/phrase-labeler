...
def parse_args():
    ...
    p.add_argument("--output", default="eval_analysis_report.html", help="Output HTML path.")
    p.add_argument("--metrics-output", help="Optional aggregated metrics JSON output path.")
    p.add_argument("--min-wrong-runs", type=int, default=None,
                   help="Default report filter: include items wrong in >N runs.")
    p.add_argument("--min-wrong-rate", type=float, default=None,
...
def analyze(files, run_ids):
        ...
        with f.open("r", encoding="utf-8") as h:
            for line_no, raw in enumerate(h, start=1):
                ...
                if error not in (None, ""):
                    ...
                    reason = "Invalid predicted label list"
                elif len(predicted) != len(expected):
                    reason = f"Prediction length mismatch (predicted={len(predicted)}, expected={len(expected)})"
    ...
    for run in sorted(run_stats):
        ...
        run_summary.append({"run_id": run, **s, "segment_accuracy": seg_acc, "exact_match_rate": exact})
...
<h1>Eval Analysis Report</h1><p class='muted'>Generated {generated_at}. Mislabel metrics include only valid model outputs. Technical failures are separate.</p>
<section class='cards'>
...
<section class='panel'><h3>Run Summary</h3><div class='table-wrap'><table><thead><tr><th>Run</th><th>Total records</th><th>Valid records</th><th>Technical failures</th><th>Exact match rate</th><th>Segment accuracy</th></tr></thead><tbody>{run_rows}</tbody></table></div></section>
...
def main():
    ...
    print(json.dumps({"output_html": str(output), "jsonl_files": len(files)}, indent=2))
